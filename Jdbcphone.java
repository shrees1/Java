    ...
    try
     {
      ...
      st=con.prepareStatement("\ninsert into Student(rollNo,name,address,course,phone) values(?,?,?,?,?)");
   ...
   else if(sh==3)
    {
    ...
    else
      {
       try
        {
         BufferedReader br=new BufferedReader(new InputStreamReader(System.in));
          System.out.println("\nEnter the roll no whose phoneno to be changed?");
           int roll=Integer.parseInt(br.readLine());
            System.out.println("\nEnter the new phoneno");
             String course=br.readLine();
 
               st=con.prepareStatement("update Student set phone= '+phone+' where rollno=?");
                st.setInt(1,roll);
                 r=st.executeUpdate();
         }
         catch(Exception e10)
